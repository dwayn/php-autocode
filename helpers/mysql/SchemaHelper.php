...
class SchemaHelper
{
	...
	protected function outputSchema()
	{
		...
		foreach($this->tables as $tablename => $thelper)
		{
			...
			if(!is_null($thelper->autoIncCol)) $tabledata['AutoIncColumn'] = $thelper->autoIncCol;
...
//			var_dump($tablename, $thelper, $joins);exit;
		...
		if(!$file) throw new SchemaHelperException("Unable to open $filename for writing");
